...
try:
    ...
    def sql_insert(conc2, entities):
        ...
        cursorObj3.execute('INSERT INTO Table_1(UserName, Password, vk_id, teleg_id) VALUES(?, ?, ?, ?)', entities)
        conc2.commit()
    ...
    def sql_update(con5, what_fetch, what_fetch_new):
        ...
        cursorObj1.execute('UPDATE Table_1 SET ' + str(what_fetch) + ' = CAST(' + "'" + str(what_fetch_new) + "'"
                           + ' AS varchar)')
        con5.commit()
    ...
    def sql_fetch(conc, what_return, vk_id):
        ...
        cursorObj1.execute('SELECT ' + str(what_return) + ' FROM Table_1 WHERE vk_id = CAST(' + "'" + str(vk_id) + "'" + ' AS varchar)')
    ...
    def sql_fetch_user(conc, what_return, UserName):
        cursorObj1 = conc.cursor()
        cursorObj1.execute('SELECT ' + str(what_return) + ' FROM Table_1 WHERE UserName = CAST(' + "'" + str(UserName) + "'" + ' AS varchar)')
        rows = cursorObj1.fetchall()
        if len(rows) == 0:
            return None
        return rows[0][0]
