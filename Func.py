    ...
    def weather(event_func):
        ...
        try:
            ...
            send_msg(event_func.message.peer_id, 'Температура в ' + str(s_city) + '\n' +
                     'Осадки: ' + str(Osadki) + '\nТемпература:\nминимальная: ' + str(Temp_min) + '\n'
                     'сейчас: ' + str(Temp) + '\nмаксимальная: ' + str(Temp_max) + '\n' +
                     'ощущается как: ' + str(Temp_fel))
        except Exception as e:
    ...
    def save_file(event_func, count_attach):
        for i in range(count_attach):  # Пробег по всем вложениям
            ...
            if content_type == "doc":  # Если это видео
                ...
                f = open(r'content/users/' + str(event_func.message.from_id) + '/doc/' + str(time.time()) +
                         '.' + str(event_func.message["attachments"][i]["doc"]["ext"]), "wb")
                ...
                f = open(r'' + str('content/users/' + str(event_func.message.from_id) + '/' + '.png'), "wb")
                ufr = requests.get(url)  # делаем запрос
